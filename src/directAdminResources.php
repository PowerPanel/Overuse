...
$username = '';
$password = '';
$server = ''; //hostname without http:// or https://
$portnumber = 2222; //portnumber
...
foreach($output["list"] as $user){
    $da = new \DirectAdmin();
    $da->connect($server, $portnumber);
    $da->set_login($username, $password);
    $da->set_method('get');
    $da->query('/CMD_API_SHOW_USER_DOMAINS', 'user='.$user);
    $da = $da->fetch_body();

    parse_str($da, $output);
	$resultarray["status"] = 'ok';
    foreach($output as $name => $website){
        list($bandwith_used, $bandwith_limit, $disk_usage, $log_usage, $subdomains, $suspended, $quota, $ssl, $cgi, $php) = explode(":", $website);
        
        $temparray["domain"] = $name;
        $temparray["traffic"] = $bandwith_used;
        $temparray["traffic_prevday"] = '';
        $temparray["subdomains"] = $subdomains;
        $temparray["emailboxen"] = '';
        $temparray["redirect"] = '';
        $temparray["webusers"] = '';
        $temparray["mailgroups"] = '';
        $temparray["responders"] = '';
        $temparray["maillists"] = '';
        $temparray["databases"] = '';
        $temparray["mssyl_databases"] = '';
        $temparray["webapps"] = '';
        $temparray["domains"] = '';
        $temparray["sites"] = '';
        $temparray["disk_usage"] = $disk_usage;
        $temparray["log_usage"] = $log_usage;
        $temparray["quota"] = $quota;
        $temparray["ssl"] = $ssl;
        $temparray["cgi"] = $cgi;
        $temparray["php"] = $php;
...
foreach($output["list"] as $user => $usage){
    $da = new \DirectAdmin();
    $da->connect($server, $portnumber);
    $da->set_login($username, $password);
    $da->set_method('get');
    $da->query('/CMD_API_SHOW_USER_USAGE', 'user='.$usage);
    $da = $da->fetch_body();

    parse_str($da, $output);

    $userarray["user"] = $usage;
    $userarray["user_bandwith"] = $output["bandwidth"];
    $userarray["user_database_quota"] = $output["db_quota"];
    $userarray["user_domain_pointer_used"] = $output["domainptr"];
    $userarray["user_email_deliveries"] = $output["email_deliveries"];
    $userarray["user_email_deliveries_incoming"] = $output["email_deliveries_incoming"];
    $userarray["user_email_deliveries_outgoing"] = $output["email_deliveries_outgoing"];
    $userarray["user_email_quota"] = $output["email_quota"];
    $userarray["user_ftp_accounts"] = $output["ftp"];
    $userarray["user_inode"] = $output["inode"];
    $userarray["user_databases"] = $output["mysql"];
    $userarray["user_used_mail_forwarders"] = $output["nemailf"];
    $userarray["user_used_maillists"] = $output["nemailml"];
    $userarray["user_used_autoresponders"] = $output["nemailr"];
    $userarray["user_used_pop_accounts"] = $output["nemails"];
    $userarray["user_used_subdomains"] = $output["nsubdomains"];
    $userarray["user_used_diskspace"] = $output["quota"];
    $userarray["user_used_virtual_domains"] = $output["vdomains"];
